...
class FeePaymentsViewPager(@NonNull fragmentActivity: FragmentActivity, private val studentProfileResponse: StudentProfileResponse?) :
    ...
    override fun createFragment(position: Int): Fragment {
        val fragment: Fragment = if (position == 0)
            FeeDetailsPagerFragment.newInstance(position, studentProfileResponse)
        else PaymentHistoryPagerFragment.newInstance(
            position, studentProfileResponse
        )
