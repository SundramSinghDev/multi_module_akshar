...
class ChildrenAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ChildViewHolder {
        return ChildViewHolder(
            parent.context,
            RowChildrenBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: ChildViewHolder, position: Int) {
        ...
        holder.binding.run {
            ...
            if (list[holder.absoluteAdapterPosition].profileImage.isNullOrBlank()) {
                ...
                tvShortName.text = list[holder.adapterPosition].studentName.substring(0, 2).uppercase()
                when (position % 3) {
                    0 -> {
            ...
            rlWhole.setOnClickListener {
                ...
                selectedItemPos = holder.absoluteAdapterPosition
                notifyItemChanged(selectedItemPos)
                Paper.book().write(USER_SELECTED_ROLE, list[holder.absoluteAdapterPosition])
     ...
     * @property binding
     */
    inner class ChildViewHolder(private val context: Context, val binding: RowChildrenBinding) :
